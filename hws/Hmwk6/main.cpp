...
bool get_words(char * file_name, vector<string> &w, char type)
{
  ...
  ifstream in;
  in.open(file_name);
  if(!in.is_open()) {std::cout << "The genome file could not be opened. Check the location.\t"; return false;}
  ...
  for(i = 1; i < (len-10-1); i++)   //read until the end of the file
  {
    ...
    word[10] = '\0';
    //strcpy(w[i],word);
    std::cout << i << "\t" << word << std::endl; std::cout.flush();
    w.push_back(word);
  }
  in.clear(); in.close();
...
bool get_reads(char * file_name, vector<string> &r, char type)
{
    ...
    ifstream in;
    in.open(file_name);
    if(!in.is_open()) {std::cout << "The read file could not be opened. Check the location.\t"; return false;}
