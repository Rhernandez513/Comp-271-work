...
bool read_file(char * file_name,std::vector<string> & w);

//this function finds if the word is already in the array and returns the index
int is_in(string w, std::vector<word_freq> & wf);

//this function finds how many times a word occurs
int get_num_occ(string w, std::vector<word_freq> & wf);
...
bool parse_line(char * word, std::vector<string> &w);
...
int main()
{
  ...
  c_filename = new char[stdstrlen(filename.c_str()) + 1];
  strcpy(c_filename, filename.c_str());
  //if file has been read and all the words are in the "words" std::vector 
  //for loop to go through std::vector words, check is word with is_in function and it should return an index
